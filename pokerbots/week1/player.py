...
class Player(Bot):
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        if opp_cards!=[]: #showdown
            ...
            for perm in self.proposal_perms:
                ...
                if my_strength==opp_strength and my_delta==0:
                    new_perms.append(perm)
            self.proposal_perms = new_perms if len(new_perms)>10 else self.proposal_perms
        if opp_cards != []:
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if FoldAction in legal_actions:
            if my_cards[0][0]==my_cards[1][0] and 0.5>(self.wins_dict[my_cards[0][0]] / self.showdowns_dict[my_cards[0][0]]):
                return FoldAction()
